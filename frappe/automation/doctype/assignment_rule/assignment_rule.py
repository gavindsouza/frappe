		...
		if user := self.get_user(doc):
			assign_to.add(
				dict(
					...
					doctype=doc.doctype,
					name=doc.name,
					description=frappe.render_template(self.description, _get_cached_dict(doc)),
					assignment_rule=self.name,
					notify=True,
					date=doc[self.due_date_based_on] if self.due_date_based_on else None,
				),
	...
	def safe_eval(self, fieldname, doc: Document):
		try:
			if evaluation_condition := getattr(self, fieldname, None):
				return frappe.safe_eval(evaluation_condition, None, _get_cached_dict(doc))
...
@request_cache
def _get_cached_dict(doc: Document):
	return doc.as_dict()
